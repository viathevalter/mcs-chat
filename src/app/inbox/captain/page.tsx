    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden w-full">
       <div className="flex flex-col flex-1 p-8 max-w-5xl mx-auto overflow-y-auto">
          ...
          <div className="flex items-center justify-between mb-8 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
             <div className="absolute right-0 top-0 bottom-0 w-64 bg-gradient-to-l from-indigo-50 to-transparent pointer-events-none opacity-50"></div>
             <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-indigo-200 rotate-3">
             ...
             <button 
                ...
                className="bg-indigo-600 text-white font-bold py-2.5 px-6 rounded-xl shadow-md shadow-indigo-200 hover:bg-indigo-700 transition-all flex items-center gap-2 relative z-10"
             >
          ...
          {!isEditing ? (
             <div className="space-y-4">
                {loading ? (
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      {agents.map(agent => (
                         <div key={agent.id} className={`p-6 bg-white rounded-3xl border ${agent.is_active ? 'border-indigo-100 shadow-sm border-l-4 border-l-indigo-500' : 'border-slate-200 opacity-70 border-l-4 border-l-slate-300'} transition-all hover:shadow-md flex flex-col justify-between group`}>
                            <div>
                               <div className="flex items-start justify-between">
                                  <div className="flex items-center gap-3 mb-4">
                                     <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${agent.is_active ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-500'}`}>
                                  ...
                                  <button onClick={() => toggleActive(agent)} className={`${agent.is_active ? 'text-indigo-600' : 'text-slate-400'} hover:opacity-80 transition-opacity`} title={agent.is_active ? "Desativar Agente" : "Ativar Agente"}>
                            ...
                            <div className="flex items-center gap-4 pt-4 border-t border-slate-100 justify-between">
                               <div className="flex items-center gap-3">
                                  ...
                                  <span className="text-[11px] font-bold px-2.5 py-1 rounded border bg-indigo-50 text-indigo-600 border-indigo-100 flex items-center gap-1">
                                     <Zap className="w-3 h-3" /> 0 Funções
                                  </span>
                               </div>
                               
                               <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button onClick={() => handleEdit(agent)} className="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
             ...
             <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 mt-2 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="flex items-center gap-4 border-b border-slate-100 pb-6 mb-8">
                   <div className="w-12 h-12 bg-indigo-100 rounded-xl flex items-center justify-center text-indigo-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                   <div>
                      ...
                      <input 
                         ...
                         className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 outline-none text-slate-800 font-medium transition-all"
                   ...
                   <div>
                      ...
                      <select 
                         ...
                         className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 outline-none text-slate-800 font-medium transition-all appearance-none cursor-pointer"
                      >
                   ...
                   <div className="md:col-span-2">
                       <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-2 flex justify-between ml-1">
                          ...
                          <span className="text-indigo-400">ContextWindow ~128K</span>
                       </label>
                       <textarea 
                          value={prompt} onChange={e => setPrompt(e.target.value)}
                          className="w-full bg-slate-50 border border-slate-200 p-5 rounded-2xl focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 outline-none text-slate-800 font-medium transition-all resize-y min-h-[250px] leading-relaxed"
                ...
                <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
                   ...
                   <button onClick={handleSave} className="px-8 py-3 font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-md shadow-indigo-200 transition-all">
